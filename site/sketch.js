...
function preload () {
  alienImage = loadImage('assets/invader1.png')
  shooterImage = loadImage('assets/player.png')
  upgradedShooterImage = loadImage('assets/playerv2.png')
}
...
async function setup () {
  ...
  const userLevel = await window.getUserLevel()

  invaders = new Invaders(alienImage, 4)
  player = new Player(shooterImage, userLevel)
...
function showGameOver () {
  ...
  gameOver = true
  fill(255)
  const gameOverT = 'GAME OVER! Click to continue. Your score was ' + player.score
  textSize(16)
...
function connectToStart () {
  ...
  const startText2 = 'Click on Connect passport'
  let textXpos1 = width / 2 - textWidth(startText1) / 2
  const textXpos2 = width / 2 - textWidth(startText2) / 2
  const textYpos = height / 2
...
async function draw () {
  if (gameOver) {
    ...
    // re-setup on user change
    if (player.profile?.email !== window.userProfile?.email) {
      await setup()
    }
