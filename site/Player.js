class Player {
  constructor (shooterImage, userLevel = 0) {
  ...
  upgradeSpaceship () {
    this.image = loadImage('assets/playerv2.png')
    this.maxBullets = 4
  }
  ...
  draw () {
    ...
    if (this.score == 100 && !this.nftShown['1']) {
      this.gamePaused = true
      this.pauseGame('1')
    } else if (this.score == 200 && !this.nftShown['2']) {
      this.gamePaused = true
      this.pauseGame('2')
    } else if (this.score == 350 && !this.nftShown['3']) {
      this.gamePaused = true
      this.pauseGame('3')
    }  else if (this.score == 550 && !this.nftShown['4']) {
      this.gamePaused = true
      this.pauseGame('4')
    }
  }
  ...
  drawGas () {
    const blocks = 8
    const blockW = this.r / 2
    const blockH = this.r / 3

    for (let i = 0; i < blocks; i++) {
      const currentW = blockW - i + 2
  ...
  drawInfo () {
    fill(255)
    const bounty_text = window?.userProfile?.email + ': '
    const bounty_text_w = textWidth(bounty_text)
    const score = text(bounty_text, 50, 25)
    push()
    fill(100, 255, 100)
